...
resource "aws_db_subnet_group" "default" {
  name       = "main"
  subnet_ids = data.terraform_remote_state.main.outputs.vpc_subnets
}
...
resource "null_resource" "cluster" {
  ...
  provisioner "file" {
    ...
    connection {
      type        = "ssh"
      host        = data.terraform_remote_state.main.outputs.ssh_host
      user        = "ubuntu"
  ...
  provisioner "remote-exec" {
    inline = [
      "mysql -h ${aws_rds_cluster_instance.master[0].endpoint} -u ${aws_rds_cluster.default.master_username} -p${aws_rds_cluster.default.master_password} < ~/search.sql",
    ]
